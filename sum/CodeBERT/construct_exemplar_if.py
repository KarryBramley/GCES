...
def construct_exemplars_if(model, args, train_exemplar, eval_exemplar, tokenizer, device, param_influence, mode):
    ...
    if task_id>0:
        ...
        for key in old_replay_train_data:
            old_replay_train_data[key].sort(key = lambda x:x["if"], reverse=True)
        with jsonlines.open(eval_replay_path) as reader:
            for obj in reader:
                if obj['task_id'] not in old_replay_valid_data:
                    old_replay_valid_data[obj['task_id']]=[obj]
        ...
        for key in old_replay_valid_data:
            old_replay_valid_data[key].sort(key = lambda x:x["if"], reverse=True)
    ...
    if mode == 'random':
        ...
        if len(train_data) < 10000:
            inverse_hvp = get_inverse_hvp_lissa(grad_one, model, args.device, param_influence, train_dataloader_lissa, damping=damping, num_samples=args.lissa_repeat, recursion_depth=int(len(train_data) * args.lissa_depth))
        else:
        ...
        for step, batch in enumerate(bar):
            ...
            ######## L_TRAIN GRADIENT ########
            model.zero_grad()
            _, train_loss, num  = model(source_ids, source_mask, target_ids, target_mask)
            train_loss = train_loss.div(num)
            train_loss.requires_grad = True
            test_grads = autograd.grad(train_loss, param_influence)
        ...
        final_score = influence_score

        new_train_exemplars = []
        for idx in train_topk:
            new_train_exemplars.append({'code_tokens':train_exemplar[idx].origin_source, 'docstring_tokens':train_exemplar[idx].origin_target, \
                                        'code':train_exemplar[idx].code, 'task_id':str(task_id), 'if':final_score[idx]})
        for idx in old_replay_train_data:
            train_exemplars.extend(old_replay_train_data[idx][:old_train_replay_size])
        train_exemplars.extend(new_train_exemplars)
        ...
        logger.info("######## START COMPUTING IHVP ########")
        # 这里lisa_dataloader的bs设置为原本的32
        eval_dataloader_lissa = DataLoader(train_data, batch_size=8, shuffle=True, drop_last=True)
        damping = args.damping
        logger.info("eval number is: ########")
        logger.info(len(eval_data))
        # 参数train_sum替换了原先代码的len(train_example)
        if len(eval_data) < 10000:
            inverse_hvp = get_inverse_hvp_lissa(grad_one, model, args.device, param_influence, eval_dataloader_lissa, damping=damping, num_samples=args.lissa_repeat, recursion_depth=int(len(eval_data) * args.lissa_depth))
        else:
            inverse_hvp = get_inverse_hvp_lissa(grad_one, model, args.device, param_influence, eval_dataloader_lissa, damping=damping, num_samples=args.lissa_repeat, recursion_depth=int(10000 * args.lissa_depth))
        logger.info("######## FINISHED COMPUTING IHVP ########")
        ...
        for step, batch in enumerate(bar):
            ...
            ######## L_TRAIN GRADIENT ########
            model.zero_grad()
            _, eval_loss, num  = model(source_ids, source_mask, target_ids, target_mask)
            eval_loss = eval_loss.div(num)
            test_grads = autograd.grad(eval_loss, param_influence)
        ...
        for idx in eval_topk:
            new_eval_exemplars.append({'code_tokens':eval_exemplar[idx].origin_source, 'docstring_tokens':eval_exemplar[idx].origin_target, \
                                        'code':eval_exemplar[idx].code, 'task_id':str(task_id), 'if':final_score[idx]})
        for idx in old_replay_valid_data:
            eval_exemplars.extend(old_replay_valid_data[idx][:old_eval_replay_size])
        eval_exemplars.extend(new_eval_exemplars)
