...
def calculate_e2ln_score(data_loader, model, tokenizer, device, args):
    ...
    for step, batch in enumerate(bar):
        ...
        with torch.no_grad():
            ...
            logits = model(input_ids=source_ids, attention_mask=source_mask, labels=labels).logits
        if step % 1000 == 0:
            logger.info(f'score batch {step + 1} of {total_batch}')
        scores.append(get_l2_norm(logits, labels))
    # print(scores)
...
def construct_exemplars_el2n(model, args, train_exemplar, eval_exemplar, tokenizer, device, mode):
    ...
    if mode == 'random':
        ...
        for clas in train_exemplar_class:
            ...
            model.eval()
            score = calculate_e2ln_score(train_dataloader, model, tokenizer, device, args)
            ...
            for idx in train_topk:
                new_train_exemplars.append({'func_before':train_exemplar[idx].origin_source, 'vul':train_exemplar[idx].origin_target, \
                                            'task_id':str(task_id), 'el2n':final_score[idx]})
