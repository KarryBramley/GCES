...
def getInstanceGradient(data_loader, model, param_influence, args):
    ...
    for step, batch in enumerate(bar):
        ...
        grads = autograd.grad(loss, param_influence)
        flatten = flatten_grads(grads)
        print(type(flatten))
        break
        if step % 1000 == 0:
    ...
    if mode == 'random':
        ...
        for clas in train_exemplar_class:
            ...
            for i in range(cluster_number):
                ...
                # start_idx = int(0.1 * class_score_size)
                start_idx = 0
                select_idx = sorted_idx[start_idx: start_idx + cluster_replay_number[i]]
