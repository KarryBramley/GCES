...
def main():
    ...
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path, ignore_mismatched_sizes=True,
                                          cache_dir=args.cache_dir if args.cache_dir else None)
    config.num_labels=1
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name,
                                                do_lower_case=args.do_lower_case, ignore_mismatched_sizes=True,
                                                cache_dir=args.cache_dir if args.cache_dir else None)
    ...
    if args.model_name_or_path:
        model = model_class.from_pretrained(args.model_name_or_path, 
                                            from_tf=bool('.ckpt' in args.model_name_or_path), ignore_mismatched_sizes=True,
                                            config=config,
                                            cache_dir=args.cache_dir if args.cache_dir else None)    
    else:
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        # delta_model = AdapterModel(backbone_model=model, modified_modules=['[r]encoder\.layer\.(\d)+'])
        for idx, model_name in enumerate(output_dir):    
            # delta_model.log() 
            # Visualization(delta_model).structure_graph()

            model.load_state_dict(torch.load(model_name),strict=False)   
            # state_dict = torch.load(model_name)
            # new_state_dict = torch.load(model_name)
            # print(idx)
            # for param in state_dict:
            #     if not param.startswith('encoder'):
            #         new_name = 'encoder.' + param
            #         new_state_dict[new_name] = state_dict[param]
            #         del new_state_dict[param]
            # if idx > 0:
