...
def train(args, train_dataset, model, tokenizer, param_influence):
    ...
    # 为了计算所有训练数据的el2n分数
    # from construct_exemplar_if import calculate_e2ln_score, get_l2_norm
    # el2n_score = calculate_e2ln_score(train_dataloader, model, args)
    # assert len(el2n_score) == len(train_dataloader.dataset)
    # if_score_path = args.output_dir + '/l2_score/'
    # if not os.path.exists(if_score_path):
    #     os.makedirs(if_score_path)   
    # pickle.dump(el2n_score, open(os.path.join(if_score_path, "el2n_score_" + str(args.task_id) + ".pkl"), "wb"))
