...
def construct_exemplars_ours(model, args, train_exemplar, eval_exemplar, tokenizer, device, mode):
    ...
    if mode == 'random':
        ...
        for clas in train_exemplar_class:
            ...
            # 保存所有训练样本的loss值
            # assert len(score) == len(train_dataloader.dataset)
            # for idx in range(len(train_dataloader.dataset)):
            #     all_train_loss_score.append({'code1':train_exemplar[idx].origin_source1, 'code2':train_exemplar[idx].origin_source2, 'label':train_exemplar[idx].origin_target, \
            #                                 'task_id':str(task_id), 'loss':score[idx]})
    ...
    # with jsonlines.open("./saved_models/emr_ours_adaewc/all_train_loss_score.jsonl", mode='w') as f:
    #     f.write_all(all_train_loss_score)
